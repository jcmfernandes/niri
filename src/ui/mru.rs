...
fn make_dynamic_opened_binds(config: &Config) -> Vec<Bind> {
    ...
    for bind in &config.binds.0 {
        let action = match &bind.action {
            ...
            | Action::FocusWindowOrGroupDown => Action::MruAdvance {
                direction: MruDirection::Forward,
                scope: None,
                filter: None,
            },
            ...
            | Action::FocusWindowOrGroupUp => Action::MruAdvance {
                direction: MruDirection::Backward,
                scope: None,
                filter: None,
            },
            Action::FocusGroupFirst => Action::MruFirst,
            Action::FocusGroupLast => Action::MruLast,
